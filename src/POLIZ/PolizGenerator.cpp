...
PolizGenerator::Generate(ASTNode* root)
{
    code_.clear();
    labelCounter_ = 0;
    breakLabels_.clear();
...
std::string PolizGenerator::NewLabel()
{
    return "L" + std::to_string(labelCounter_++);
}

void PolizGenerator::GenNode(ASTNode* node)
{
    if (!node)
        return;
...
void PolizGenerator::GenFunction(ASTNode* node)
{
...
void PolizGenerator::GenStatement(ASTNode* node)
{
    if (!node)
        return;
...
void PolizGenerator::GenIf(ASTNode* node)
{
...
void PolizGenerator::GenExpression(ASTNode* node)
{
    if (!node)
        return;
